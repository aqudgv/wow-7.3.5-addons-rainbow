...
function WeakAuras.LoadOptions(msg)
  if not(IsAddOnLoaded("WeakAurasOptions")) then
    if InCombatLockdown() then
      ...
      print("|cff9900FF"..L["WeakAuras Options"]..FONT_COLOR_CODE_CLOSE..L[" will finish loading after combat."])
      queueshowooc = msg or "";
      ...
      if not(loaded) then
        print("|cff9900FF"..L["WeakAuras Options"]..FONT_COLOR_CODE_CLOSE..L[" could not be loaded: "]..RED_FONT_COLOR_CODE.._G["ADDON_"..reason]);
        return false;
      end
    end
  end
...
local function ensureMouseFrame()
  ...
  label:SetText(L["WeakAuras Anchor"]);
