...
function RE:OnLoad(self)
	...
	_G.REFlexFrame_Title:SetText(L["REFlex "]..tostring(RE.Version):gsub(".", "%1."):sub(1,-2))
...
function RE:OnEvent(_, event, ...)
	if event == "ADDON_LOADED" and ... == "REFlex" then
		...
		RE.OptionsMenu = _G.LibStub("AceConfigDialog-3.0"):AddToBlizOptions("REFlex", L["REFlex"])
		RegisterAddonMessagePrefix("REFlex")
		_G.BINDING_HEADER_REFLEXB = L["REFlex"]
		_G.BINDING_NAME_REFLEXOPEN = L["Show main window"]

		TOAST:Register("REFlexToast", function(toast, ...)
			toast:SetFormattedTitle(L["|cFF74D06CRE|r|cFFFFFFFFFlex|r"])
