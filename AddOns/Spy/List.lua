	...
	if expired then
		...
		if Spy.db.profile.HideSpy and Spy:GetNearbyListSize() == 0 then 
			if not InCombatLockdown() then
				Spy.MainWindow:Hide()
--				print("[Spy] MainWindow Hidden... Not in combat and list = 0.")				
			else	
	...
	if playerData then
		if not reason then
			...
			if reason == L["KOSReasonOther"] then
				if not other then --next 4 lines added back
...
--						DEFAULT_CHAT_FRAME:AddMessage(Spy.db.profile.SelectWarnRace.." Detected... "); --+++
...
function Spy:AlertStealthPlayer(player)
	if Spy.db.profile.WarnOnStealth then
		if Spy.db.profile.DisplayWarningsInErrorsFrame then
			...
			UIErrorsFrame:AddMessage(msg, text.r, text.g, text.b, 1.0, UIERRORS_HOLD_TIME)						
			--UIErrorsFrame:AddMessage(msg, text.r, text.g, text.b, 1, 9)
		else
