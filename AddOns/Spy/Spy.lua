...
Spy.optionsSlash = {
	...
	args = {
		...
		kos = {
			...
			type = 'input',
			order = 5,
--			pattern = "%a",  --++ Changed so names with special characters can be added (i.e. Russian, Chinese...)
			pattern = ".",			
			set = function(info, value)
...
--	self.optionsFrames[name] = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("Spy", displayName, "Spy Option", name)	
end
...
--	self.optionsFrames.Spy = ACD3:AddToBlizOptions("Spy", "Spy Option", nil, "General")	
...
--function Spy:BuildZoneIDTable_z(continentIndex, zoneIndex, zoneName, next, ...) -- WoD Change
function Spy:BuildZoneIDTable_z(continentIndex, zoneIndex, zoneId, zoneName, next, ...) -- WoD Change
	zoneIndex = zoneIndex + 1
--	DEFAULT_CHAT_FRAME:AddMessage("###" .. zoneIndex .. ": " .. zoneId .. "- " .. zoneName); --Test output
	Spy.ZoneID[zoneName] = {}
	Spy.ZoneID[zoneName].continentIndex = continentIndex
	Spy.ZoneID[zoneName].zoneIndex = zoneIndex
	Spy.ZoneID[zoneName].continentId = continentId	-- WoD Change
	Spy.ZoneID[zoneName].zoneId = zoneId  -- WoD Change
	if next then Spy:BuildZoneIDTable_z(continentIndex, zoneIndex, next, ...) end
end

--function Spy:BuildZoneIDTable(zoneIndex, continentName, next, ...) -- WoD Change
function Spy:BuildZoneIDTable(zoneIndex, continentId, continentName, next, ...) -- WoD Change
	zoneIndex = zoneIndex + 1
--	DEFAULT_CHAT_FRAME:AddMessage("#" .. zoneIndex .. ": " .. continentId .. "- " .. continentName); --Test output
	Spy:BuildZoneIDTable_z(continentId, 0, GetMapZones(zoneIndex)) -- WoD Change
--	Spy:BuildZoneIDTable_z(zoneIndex, 0, GetMapZones(continentId)) -- WoD Change
...
--	return Spy.ZoneID[zoneName].continentIndex, Spy.ZoneID[zoneName].zoneIndex  -- WoD Change
	return Spy.ZoneID[zoneName].continentIndex, Spy.ZoneID[zoneName].zoneId  -- WoD Change
end
...
--			print("Channel Changed: " .. Spy.zName)
			Spy.ChnlTime = time()
--			print(Spy.ChnlTime)
			Spy.EnabledInZone = false
		end	
...
--	print("Player Entering World Event: " .. zone)
	local now = time()
	if Spy.ChnlTime > (now - 6) then	
--		print("[Spy] Delayed for ... 6 seconds while Player Entering World - Dalaran.")		
...
function Spy:ZoneChangedEvent()
 	local zone = GetZoneText()
--	print("Zone Changed Event: " .. zone)
	local now = time()
	if Spy.ChnlTime > (now - 6) then	
--		print("[Spy] Delayed for ... 6 seconds while Zone Changed - Dalaran.")		
...
function Spy:ZoneChangedNewAreaEvent()
 	local zone = GetZoneText()
--	print("Zone Changed New Area Event: " .. zone)
	local now = time()
	if Spy.ChnlTime > (now - 6) then	
--		print("[Spy] Delayed for ... 6 seconds while Zone Changed New Area - Dalaran.")		
		self:ScheduleTimer("PlayerEnteringWorldEvent",6)
		return		
	else 
		Spy:ZoneChanged()		
...
--	if pvpType == "sanctuary" or GetZoneText() == "" or GetSubZoneText() == "The Vindicaar" then --++ chg so Spy is not active in the Vindicaar 	
...
function Spy:CombatLogEvent(info, timestamp, event, hideCaster, srcGUID, srcName, srcFlags, sourceRaidFlags, dstGUID, dstName, dstFlags, destRaidFlags, ...)
	if Spy.EnabledInZone then
		...
		local _, overkill 	
		overkill = 0;		--petKill test code end		

--		DEFAULT_CHAT_FRAME:AddMessage("Beta Test msg CombatLog 1");
...
--			DEFAULT_CHAT_FRAME:AddMessage(srcType .. " CombatLog Src Test msg1");
--			print (srcType)
--			if srcType == 0 or srcType == 8 then
			if srcType == "Player" then
--			DEFAULT_CHAT_FRAME:AddMessage(srcType .. " CombatLog Src Test msg2");
				local learnt = false
				local detected = true
				...
				if detected then
					...
					if event == "SPELL_AURA_APPLIED" and (arg10 == L["Prowl"]) then
...
--			DEFAULT_CHAT_FRAME:AddMessage(dstType .. " CombatLog Dst Test msg");
--			if dstType == 0 or dstType == 8 then
		...
		--PetKill code start here
		-- update win statistics for pet kills
		if (combatEvent[event] and srcName == petName) then		
			if event == "SWING_DAMAGE" then
				_, overkill = ...
--				DEFAULT_CHAT_FRAME:AddMessage("pet kill event 1: " .. event .. " " ..overkill);				
			else
				_, _, _, _, overkill = ...
--				DEFAULT_CHAT_FRAME:AddMessage("pet kill event 2: " .. event .. " " ..overkill);
			end
...
--++				if tonumber(version) > tonumber(Spy.Version) and not Spy.UpgradeMessageSent then
					...
					if strlen(race) > 0 then
						...
						if (Spy.EnemyFactionName == "Alliance" and race ~= "Draenei" and race ~= "Dwarf" and race ~= "Gnome" and race ~= "Human" and race ~= "Pandaren" and race ~= "Night Elf" and race ~= "Worgen" and race ~= "Void Elf" and race ~= "Lightforged Draenei") --++
						or (Spy.EnemyFactionName == "Horde" and race ~= "Blood Elf" and race ~= "Goblin" and race ~= "Orc" and race ~= "Pandaren" and race ~= "Tauren" and race ~= "Troll" and race ~= "Undead" and race ~= "Nightborne" and race ~= "Highmountain Tauren") then --++
							return
						end
					else
						race = nil
...
--		DEFAULT_CHAT_FRAME:AddMessage("#" .. currentContinentIndex .. ": " .. currentZoneIndex); --Test Results
		local continentIndex, zoneIndex = Spy:GetZoneID(playerData.zone)
--		DEFAULT_CHAT_FRAME:AddMessage("##" .. continentIndex .. ": " .. zoneIndex); --Test Resutls
		...
		if continentIndex ~= nil and zoneIndex ~= nil and type(playerData.mapX) == "number" and type(playerData.mapY) == "number" and (Spy.db.profile.MapDisplayLimit == "None" or (Spy.db.profile.MapDisplayLimit == "SameZone" and zoneIndex == currentZoneIndex) or (Spy.db.profile.MapDisplayLimit == "SameContinent" and continentIndex == currentContinentIndex)) then
			...
			Astrolabe:PlaceIconOnWorldMap(WorldMapDetailFrame, note.worldIcon, zoneIndex, 0, mapX, mapY) -- WoD Change
			Astrolabe:PlaceIconOnMinimap(note.miniIcon, note.zoneIndex, 0, note.mapX, note.mapY)	-- WoD Change
...
--		print("[Spy] Delayed for...10 seconds while checking combat status.")
		self:ScheduleTimer("HideSpyCombatCheck",10)
		return
	else
		Spy.MainWindow:Hide()
--		print("[Spy] MainWindow HiddenCheck... Not in combat and list = 0.")	
	end
end
