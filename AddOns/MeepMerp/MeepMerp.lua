...
soundFile = "Interface\\AddOns\\MeepMerp\\Bonk.ogg" -- 音效檔案位置和檔名
...
function MeepMerp:COMBAT_LOG_EVENT_UNFILTERED(event, timestamp, typeOfEvent, hideCaster, sourceGUID, sourceName, _, _, _, _, _, _, _, _, _, failureReason)
	local playerName = UnitName("player");
	local playerVehicleName = UnitName("vehicle");

	if (sourceName == playerName or sourceName == playerVehicleName) then
		if typeOfEvent == "SPELL_CAST_FAILED" then
