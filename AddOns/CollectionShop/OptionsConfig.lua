	...
	subFrameTabs = {
		...
		},
		{
			...
			Refresh			= function( SubFrame )
				...
				_G[sfn .. "AuctionsWonText"]:SetText( string.format( L["%s\n%sBuyout tracking is reset when closing %s|r"], #NS.auctionsWon .. " " .. GREEN_FONT_COLOR_CODE .. ( #NS.auctionsWon == 1 and L["Buyout"] or L["Buyouts"] ) .. FONT_COLOR_CODE_CLOSE .. ( NS.copperAuctionsWon > 0 and " (" .. NS.MoneyToString( NS.copperAuctionsWon, HIGHLIGHT_FONT_COLOR_CODE ) .. ")" or "" ), RED_FONT_COLOR_CODE, L[NS.title] ) );
			end,
		},
		{
			-- GetAll Scan Data
			...
			Init			= function( SubFrame )
				...
				NS.Button( "DeleteDataButton", SubFrame, L["Delete Data"], {
					...
					OnClick = function()
						StaticPopup_Show( "CS_REALM_DELETE", UIDropDownMenu_GetSelectedValue( _G[SubFrame:GetName() .. "RealmDropDownMenu"] ), L[NS.title], { ["realmName"] = UIDropDownMenu_GetSelectedValue( _G[SubFrame:GetName() .. "RealmDropDownMenu"] ) } );
					end,
				} );
		...
		},
		{
			...
			tabText			= HELP_LABEL,
			Init			= function( SubFrame )
				NS.TextFrame( "Description", SubFrame, string.format( L["%s version %s"], L[NS.title], NS.versionString ), {
					setPoint = {
						{ "TOPLEFT", "$parent", "TOPLEFT", 8, -8 },
						{ "RIGHT", -8 },
					},
