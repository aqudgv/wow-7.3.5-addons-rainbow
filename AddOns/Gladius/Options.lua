...
SlashCmdList["GLADIUS"] = function(msg)
	if msg:find("test") or msg:find("測試") and not Gladius.test then
	...
	elseif msg == "hide" or ((msg:find("test") or msg:find("測試")) and Gladius.test) then
		if InCombatLockdown() then
			return
		end
...
function Gladius:SetupOptions()
	self.options = {
		type = "group",
		name = L["Gladius"].." ".."Gladius "..GetAddOnMetadata("Gladius", "Version"),
		plugins = { },
		get = getOption,
		set = setOption,
		args = {
	...
	LibStub("AceConfigDialog-3.0"):AddToBlizOptions("Gladius", L["Gladius"])
end