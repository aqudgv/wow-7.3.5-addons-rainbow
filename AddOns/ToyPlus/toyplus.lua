...
function ToyPlus:RegisterOptions()-- Blizzard Options
	...
	AceConfig:RegisterOptionsTable("玩具", {
		type = 'group',
		args = {
			toyCols = {
				type = 'range',
	...
	LibStub("AceConfigDialog-3.0"):AddToBlizOptions("玩具")
end
...
function ToyPlus:Launch()
	if InCombatLockdown() then DEFAULT_CHAT_FRAME:AddMessage("玩具選單: 錯誤: \124cffffffff戰鬥中無法執行這個動作。", 0, 0.7, 1); return end
	if C_ToyBox.GetToyFromIndex(1) ~= -1 then
		...
		DEFAULT_CHAT_FRAME:AddMessage("玩具選單: 錯誤: \124cffffffff尚未載入玩具箱或無法找到玩具。", 0, 0.7, 1)
	end
end
			...
			chkbox:SetScript("OnClick", function(self, button)
				if button == "LeftButton" then
					if InCombatLockdown() then 
						DEFAULT_CHAT_FRAME:AddMessage("玩具選單: 錯誤:\124cffffffff 戰鬥中無法執行這個動作。", 0, 0.7, 1)
						...
						if isFavorite then
							local menu = {
							    { text = "Remove Favorite", func = function()
							        if InCombatLockdown() then DEFAULT_CHAT_FRAME:AddMessage("玩具選單: 錯誤:\124cffffffff 戰鬥中無法執行這個動作。", 0, 0.7, 1); return end
...
function ToyPlus:CreateFrame()
	...
	btnAddRow:SetScript("OnClick", function (self, button) 
		if button == "LeftButton" then
			if InCombatLockdown() then DEFAULT_CHAT_FRAME:AddMessage("玩具選單: 錯誤:\124cffffffff 戰鬥中無法調整列數。", 0, 0.7, 1) return end
