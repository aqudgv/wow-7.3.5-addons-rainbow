		...
		if HiddenArtifactTracker.active and arti_names[name] then
			...
			if IsQuestFlaggedCompleted(arti_names[name]) or HiddenArtifactTracker.forceTracking then
				...
				if a<b then
					if HiddenArtifactTracker.colourOptions then
						GameTooltip:AddLine(n_once.."地城："..a.."/"..b,math.min(1,2-(2*a/b)),math.min(1,2*a/b),0,True)
					else
						GameTooltip:AddLine(n_once.."地城："..a.."/"..b,1,1,1,True)
					end
					n_once=""
				end
				if c~=d then
					if HiddenArtifactTracker.colourOptions then
						GameTooltip:AddLine(n_once.."世界任務："..c.."/"..d,math.min(1,2-(2*c/d)),2*c/d,0,True)
					else
						GameTooltip:AddLine(n_once.."世界任務："..c.."/"..d,1,1,1,True)
					end
					n_once=""
				end
				if e~=f then
			...
			if HiddenArtifactTrackerChars.trackBosses and HiddenArtifactTrackerChars[name] ~= nil and IsQuestFlaggedCompleted(HiddenArtifactTrackerChars[name]["quest"]) then
				...
				if HiddenArtifactTrackerChars[name]["completion"] < 10 then
					...
					if HiddenArtifactTracker.colourOptions then
						...
						GameTooltip:AddLine("法師塔："..HiddenArtifactTrackerChars[name]["completion"].."/10", 2-2*col, 2*col, 0, True)
					else
						GameTooltip:AddLine("法師塔："..HiddenArtifactTrackerChars[name]["completion"].."/10", 1,1,1, True)
					end
					...
					for k,v in pairs(HiddenArtifactTrackerChars[name]) do
						if k ~= "rbg" and k ~= "completion" and k ~= "quest" then
							if v==false then
								...
								GameTooltip:AddLine(localisedDungeons[k].."蓋好了。", 1,1,1,True)
							end
				...
				if deltaWins < 10 then
					...
					if HiddenArtifactTracker.colourOptions then
						...
						GameTooltip:AddLine("法師塔勝利積分："..deltaWins.."/10", 2-2*col, 2*col, 0, True)	
					else
						GameTooltip:AddLine("法師塔勝利積分："..deltaWins.."/10", 1,1,1, True)	
					end
				end
			...
			elseif HiddenArtifactTrackerChars.trackBosses and HiddenArtifactTrackerChars[name] ~= nil and IsQuestFlaggedCompleted(HiddenArtifactTrackerChars[name]["quest"]) == false then
				...
				if state == Enum.ContributionState.Active or state == Enum.ContributionState.UnderAttack then
					GameTooltip:AddLine("法師塔蓋好了!", col, 1, col, True)
				elseif state == Enum.ContributionState.Destroyed then
					GameTooltip:AddLine("法師塔被摧毀了!", 1, col, col, True)			
				else
					GameTooltip:AddLine("法師塔進度 "..perc.."%", 1,col,col,True)
				end
			end
