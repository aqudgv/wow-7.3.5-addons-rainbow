...
local function FormatTime(seconds)
	...
	elseif seconds < aboutHour then
		return "%d分", round(seconds/minute)
	elseif seconds < aboutDay then
		return "%d小時", round(seconds/hour)
	else
		return "%d天", round(seconds/day)
	end
end
...
local function Initialize()
	...
	fPBMainOptions = dialog:AddToBlizOptions(AddonName, L[AddonName])

	config:RegisterOptionsTable(AddonName.." Spells", fPB.SpellsTable)
	fPBSpellsList = dialog:AddToBlizOptions(AddonName.." Spells", L["Specific spells"], L[AddonName])

	config:RegisterOptionsTable(AddonName.." Profiles", LibStub("AceDBOptions-3.0"):GetOptionsTable(fPB.db))
	fPBProfilesOptions = dialog:AddToBlizOptions(AddonName.." Profiles", L["Profiles"], L[AddonName])
