...
local BANKITEMS_VERSIONTEXT	= L["BankItems"].." v"..BANKITEMS_VERSION
...
local GetRecipeReagentItemLink, GetQuestLogItemLink, GetQuestItemLink = C_TradeSkillUI.GetRecipeReagentItemLink, GetQuestLogItemLink, GetQuestItemLink
...
function BankItems_CreateFrames()
	...
	temp:SetScript("OnClick", function()
		InterfaceOptionsFrame_OpenToCategory(L["Bank"])
		InterfaceOptionsFrame_OpenToCategory(L["Bank"])
	end)
...
function BankItems_Chat(msg)
	if DEFAULT_CHAT_FRAME then
		DEFAULT_CHAT_FRAME:AddMessage(L["<BankItems> "]..msg, 1, 1, 0)
	end
end
...
do
	...
	BankItems_OptionsFrame.name = L["Bank"]
