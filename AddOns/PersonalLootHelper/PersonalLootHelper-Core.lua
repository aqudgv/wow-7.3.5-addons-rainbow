		...
		if isAnUpgradeForAnyCharacter then
			...
			if PLH_NOTIFY_GROUP then
				if not PLH_IsInLFR() then
					PLH_SendBroadcast(PLH_GetNameWithoutRealm(characterName) .. ' 拾取的裝備 ' .. item .. ' 可以交易，會幫助 ' .. names .. ' 提升裝等！')
				end
			end

			if characterName == PLH_GetUnitNameWithRealm('player') then  -- player can trade an item
				PLH_SendAlert('你拾取的裝備 ' .. item .. ' 可以交易，會幫助 ' .. names .. ' 提升裝等！')
			...
			elseif IsPlayerInUpgradeList(isAnUpgradeForAnyCharacterNames) then  -- player can receive an item
				PLH_SendAlert(PLH_GetNameWithoutRealm(characterName) .. ' 拾取的裝備 ' .. item .. ' 可以交易，會幫助你提升裝等！')
...
local function AskForRolls()
	if currentRollItem == nil and numOfQueuedRollItems > 0 then
		...
		PLH_SendBroadcast('需要 ' .. currentRollItem .. description .. ' 的請骰 (由  ' .. PLH_GetNameWithoutRealm(currentRollOwner) .. ' 熱心提供)', true)
...
--			elseif delay >= 15 and not FifteenSecondWarningDisplayed then
			if delay >= 15 and not FifteenSecondWarningDisplayed then
				PLH_SendBroadcast(currentRollItem .. ' 還有 15 秒...', false)
				FifteenSecondWarningDisplayed = true
			elseif delay >= 25 and not FiveSecondWarningDisplayed then
				PLH_SendBroadcast(currentRollItem .. ' 還有 5 秒...', false)
...
function PLH_EndRolls()
	...
	if winners ~= nil then
		if #winners > 1 then
			PLH_SendBroadcast(GetNames(winners) .. ' 以 ' .. highRoll .. ' 點贏得 ' .. currentRollItem .. '，請找 ' .. PLH_GetNameWithoutRealm(currentRollOwner) .. '交易。', true)
		else
			PLH_SendBroadcast(winners[1] .. ' 以 ' .. highRoll .. ' 點贏得 ' .. currentRollItem .. '，請找 ' .. PLH_GetNameWithoutRealm(currentRollOwner) .. '交易。', true)
		end
	else
		PLH_SendBroadcast(PLH_GetNameWithoutRealm(currentRollOwner) .. ' 提供的 ' .. currentRollItem .. ' 沒有人骰。', true)
	end
	...
	if PLH_COORDINATE_ROLLS and PlayerCanCoordinateRolls() then
		...
		if whisperedItem ~= nil or message == TRADE_MESSAGE or message == '\'' .. TRADE_MESSAGE .. '\'' then
			if whisperedItems[sender] ~= nil then
				...
				if currentRollItem ~= nil then
					PLH_SendWhisper('感謝大大！ ' .. item .. ' 會在目前骰裝結束後開始骰。', sender)
				else 
					AskForRolls()
				end
			elseif currentRollOwner == sender then
				PLH_SendWhisper('你的 ' .. currentRollItem .. ' 已經開始骰了。', sender)
			else
				local item = GetItemFromQueueByPlayer(sender)
				if item then
					PLH_SendWhisper('你的 ' .. item .. ' 已經加入骰裝的等候佇列。', sender)
				else
					PLH_SendWhisper('沒有你拾取裝備的記錄！如果你仍然想要交易裝備，可以密 ' .. UnitName('player') .. ' \'交易 [裝備連結]\' ', sender)
				end
			end
		end
	end
...
--			local name, roll, minRoll, maxRoll = message:match('^(.+) rolls (%d+) %((%d+)%-(%d+)%)$')
			...
			if name then
				...
				if minRoll ~= '1' or maxRoll ~= '100' then
					PLH_SendBroadcast(name .. ' 骰 ' .. minRoll .. ' - ' .. maxRoll .. '；不算數。', false)
				elseif currentRolls[name] ~= nil then
					PLH_SendBroadcast(name .. ' 骰了太多次；只算第一次骰的 ' .. currentRolls[name] .. '。', false)
				elseif fullname ~= nil and not IsEquippableItemForCharacter(GetFullItemInfo(currentRollItem), fullname, false) then
					PLH_SendBroadcast(name .. ' 不適合使用 ' .. currentRollItem .. '；不算數。', false)
				else 
	...
	elseif msg == 'endroll' then
		if currentRollItem ~= nil then
			if nextRollDelay > 0 then
				PLH_SendUserMessage('請稍後...等待下一次骰裝。')
			else
...
--		PLH_UnitTest()
		...
		PLH_SendUserMessage('未知的指令。可以使用的指令為：\n  [/plh]  ：  開啟設定選項。\n  [/plh endroll]  ：  強制結束目前的骰裝。')
	end
end
