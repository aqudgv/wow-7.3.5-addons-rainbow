...
e.FONT.CONTENT = "Fonts\\bLEI00D.TTF"
e.FONT.SIZE = 16
e.FONT.OBJECT = {}
...
function e.CreateHeader(parent, name, width, height, text, fontAdjust, fontFace)
	-- 加入指定字型的參數 font
	if (fontFace == nil or fontFace == "") then fontFace = FONT_HEADER end
	local self = parent:CreateFontString('BACKGROUND')
	self:SetWidth(width)
	self:SetFont(fontFace, FONT_SIZE + fontAdjust)
	self:SetText(text)
