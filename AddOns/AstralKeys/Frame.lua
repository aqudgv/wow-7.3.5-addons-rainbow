...
local function CreateCharacterFrame(parent, frameName, unitName, bestKey, createDivider)
	...
	frame:SetScript('OnEnter', function(self)
		...
		if self.weeklyAP > 0 then
			...
			astralMouseOver:SetText('本週尚未打過M+')
		end
...
AstralMenuFrame:AddSelection('密語', SendWhisper, Whisper_OnShow)
...
AstralMenuFrame:AddSelection('邀請', InviteUnit, Invite_OnShow)

AstralMenuFrame:AddSelection('取消', function() return AstralMenuFrame:Hide() end)
...
guildInfo:SetScript('OnMouseDown', function(self) 
	if not astralGuildInfo then
		...
		astralGuildInfo.text:SetText('造訪 Astral 網站')
		...
		astralGuildInfo.button = CreateButton(astralGuildInfo, nil, 40, 20, '確定', FONT_OBJECT_CENTRE, FONT_OBJECT_HIGHLIGHT)
...
friendButton:SetText(WrapTextInColorCode('好友名單', 'ff9d9d9d'))
...
local affixHeader = e.CreateHeader(affixFrame, 'affixHeader', 175, 20, '詞綴', 12, FONT_CONTENT)
...
characterFrame:SetPoint('TOPLEFT', affixFrame, 'BOTTOMLEFT', 0, -20)

local characterHeader = e.CreateHeader(characterFrame, 'affixHeader', 175, 20, '角色', 10, FONT_CONTENT)
...
local keyButton = CreateButton(contentFrame, 'keyButton', 45, 20, '層數', FONT_OBJECT_CENTRE, FONT_OBJECT_HIGHLIGHT) --75
...
local mapButton = CreateButton(contentFrame, 'mapButton', 170, 20, '地城', FONT_OBJECT_CENTRE, FONT_OBJECT_HIGHLIGHT)
...
local nameButton = CreateButton(contentFrame, 'nameButton', 165, 20, '玩家', FONT_OBJECT_CENTRE, FONT_OBJECT_HIGHLIGHT)
