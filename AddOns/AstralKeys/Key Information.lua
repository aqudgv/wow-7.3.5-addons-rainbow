...
local function Weekly()
	e.GetBestClear()
	-- 暫時修正
	if AstralCharacters[e.GetCharacterID(e.Player())] and AstralCharacters[e.GetCharacterID(e.Player())].level >= e.CACHE_LEVEL then
		if IsInGuild() then
...
function e.CreateKeyLink(mapID, keyLevel)
	return strformat('\124cffa335ee\124Hkeystone:%d:%d:%d:%d:%d|h[傳奇鑰石：%s]\124h\124r', mapID, keyLevel, e.AffixOne(), e.AffixTwo(), e.AffixThree(), e.GetMapName(mapID))--:gsub('\124\124', '\124')
end
