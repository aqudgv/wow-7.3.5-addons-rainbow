		...
		for i = index, numEncounters + index - 1 do
			...
			if isDead then
                ...
                if bossName == nil then
                    bossName = "bossname"
                    encounterLine.text = "已拾取 - "..bossName
                else 
				    encounterLine.text = "已拾取 - "..bossName
                ...
                if bossName == nil then
                    bossName = "bossname"
                    encounterLine.text = "可拾取 - "..bossName
                else 
				    encounterLine.text = "可拾取 - "..bossName
                end
			end
...
function addon:ShowTooltip(button)
	...
	if ENABLE_COLORBLIND_MODE == "1" then -- TODO: Remove duplicates. Only check for not ENABLE_COLORBLIND_MODE in which case add the first AddLine. After outside of if, add second AddLine
		GameTooltip:AddLine("(按鈕亮起表示你已經在排這個團隊)")
	else
		GameTooltip:AddLine("(|c00ff0000紅色|r表示首領已經被擊殺/已拾取過戰利品，|n |c0000ff00綠色|r表示還活著/可以拾取戰利品，")
		GameTooltip:AddLine(" 按鈕亮起表示你已經在排這個團隊)")
	end
	GameTooltip:Show()
end
