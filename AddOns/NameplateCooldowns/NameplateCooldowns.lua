...
local CONST_SORT_MODES_L = {
	...
	[CONST_SORT_MODES[2]] = L["trinkets, then interrupts, then other spells"],
	[CONST_SORT_MODES[3]] = L["interrupts, then trinkets, then other spells"],
	[CONST_SORT_MODES[4]] = L["trinkets, then other spells"],
	[CONST_SORT_MODES[5]] = L["interrupts, then other spells"],
};
...
do
	...
	function AddButtonToBlizzOptions()
		...
		frame.name = L["NameplateCooldowns"];
...
do
	...
	function GUICategory_1(index, value)
		...
		dropdownIconSortMode.text:SetText(L["Sort mode:"]); -- // todo:localize
...
do
	...
	function Print(...)
		...
		DEFAULT_CHAT_FRAME:AddMessage(format(L["NameplateCooldowns"].."：%s", text), 0, 128, 128);
	end
