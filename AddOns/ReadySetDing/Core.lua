...
function RSD:OnInitialize()
	...
	ACD:AddToBlizOptions("ReadySetDing_Parent", L.OPT_NAME)
	...
	for _, v in ipairs(appKey) do
		...
		ACD:AddToBlizOptions(v, appValue[v].name, L.OPT_NAME)
	end
				...
				if secondGRU then
					...
					if profile.GuildChangelog then
						if not showedHeader and char.LastCheck then
							self:Print(format("公會成員升級 |cffF6ADC6[%s]|r - |cffADFF2F[%s]|r", char.LastCheck, date("%Y.%m.%d %H:%M:%S")))
							showedHeader = true; char.LastCheck = nil
						end
						print(format("|cff%s|H玩家:%s|h[%s]|h|r %s |cffF6ADC6%s|r - |cffADFF2F%s|r (+|cff71D5FF%s|r)",
