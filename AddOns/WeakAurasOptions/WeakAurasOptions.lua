...
function WeakAuras.ToggleOptions(msg)
  ...
  elseif (InCombatLockdown()) then
    print("|cff9900FF"..L["WeakAuras Options"]..FONT_COLOR_CODE_CLOSE..L[" will open after combat."])
    reopenAfterCombat = true;
  else
...
function WeakAuras.OpenModelPicker(data, field)
  if not(IsAddOnLoaded("WeakAurasModelPaths")) then
    ...
    if not(loaded) then
      print("|cff9900FF"..L["WeakAurasModelPaths"]..FONT_COLOR_CODE_CLOSE..L[" could not be loaded: "]..RED_FONT_COLOR_CODE.._G["ADDON_"..reason]);
...
function WeakAuras.OpenTriggerTemplate(data)
  if not(IsAddOnLoaded("WeakAurasTemplates")) then
    ...
    if not(loaded) then
      print("|cff9900FF"..L["WeakAurasTemplates"]..FONT_COLOR_CODE_CLOSE..L[" could not be loaded: "]..RED_FONT_COLOR_CODE.._G["ADDON_"..reason]);
      return;
    end
