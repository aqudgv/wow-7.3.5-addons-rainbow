...
function SlashCmdList.ICYDEMONSINDICATOR(msg)
    ...
    elseif action == 'y' then
		...
		if (GetLocale() == 'zhTW') then
            DEFAULT_CHAT_FRAME:AddMessage('|cff00FF00IcyDemonsIndicator|r 術士惡魔監控插件用法說明')
            DEFAULT_CHAT_FRAME:AddMessage('指令：/idi 或 /icydemonsindicator')
            DEFAULT_CHAT_FRAME:AddMessage('/idi width 100：進度條寬度設為 100（預設值 100）')
            DEFAULT_CHAT_FRAME:AddMessage('/idi height 20：進度條高度設為 20（預設值 20）')
            DEFAULT_CHAT_FRAME:AddMessage('/idi x 0：水平位置設為 0（預設值 0）')
            DEFAULT_CHAT_FRAME:AddMessage('/idi y -80：垂直位置設為 -80（預設值 -80）')
